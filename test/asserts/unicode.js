...
QUnit.test("Text file with long unicode string", function() {
    ...
    .then(function (zip) {
        ...
        QUnit.equal(fileContent, expected, "Generated ZIP can be parsed");
        QUnit.start();
    })['catch'](JSZipTestUtils.assertNoError);
});
...
JSZipTestUtils.testZipFile("Zip text file with non unicode characters in filename: loadAsync without decodeFileName", "ref/local_encoding_in_name.zip", function(content) {
    ...
    JSZip.loadAsync(content).then(function (zipUnicode) {
        QUnit.ok(!zipUnicode.files["Новая папка/"], "default : the folder is not found");
        QUnit.ok(!zipUnicode.files["Новая папка/Новый текстовый документ.txt"], "default : the file is not found");
        QUnit.start();
    })['catch'](JSZipTestUtils.assertNoError);
...
JSZipTestUtils.testZipFile("Zip text file with non unicode characters in filename: loadAsync with decodeFileName", "ref/local_encoding_in_name.zip", function(content) {
    ...
    JSZip.loadAsync(content, {
        ...
        QUnit.ok(zipCP866.files["Новая папка/"], "with decodeFileName : the folder has been correctly read");
        QUnit.ok(zipCP866.files["Новая папка/Новый текстовый документ.txt"], "with decodeFileName : the file has been correctly read");
        QUnit.start();
    })['catch'](JSZipTestUtils.assertNoError);
});
...
JSZipTestUtils.testZipFile("Zip text file and UTF-8, Pile Of Poo test", "ref/pile_of_poo.zip", function(expected) {
    ...
    zip.generateAsync({type:"binarystring", comment : text}).then(function(actual) {
        ...
        .then(function (zip) {
            ...
            QUnit.ok(file, "JSZip finds the unicode file name on the external file");
            QUnit.equal(file.comment, text, "JSZip can decode the external file comment");
            return file.async("string");
        })
        .then(function (content) {
            QUnit.equal(content, text, "JSZip can decode the external file");
            QUnit.start();
        })['catch'](JSZipTestUtils.assertNoError);
        ...
        .then(function (zip) {
            var file = zip.file(text + ".txt");
            QUnit.ok(file, "JSZip finds the unicode file name on its own file");
            QUnit.equal(file.comment, text, "JSZip can decode its own file comment");
            return file.async("string");
        })
        .then(function (content) {
            QUnit.equal(content, text, "JSZip can decode its own file");
            QUnit.start();
        })['catch'](JSZipTestUtils.assertNoError);
