...
function ZipFileWorker(streamFiles, comment, platform, encodeFileName, encryptOptions) {
    ...
    // The Aes encryption options if add password in options.
...
ZipFileWorker.prototype.push = function (chunk) {
    ...
    if (this.accumulate) {
        ...
        if (this.streamFiles && this.encryptOptions.password !== null && chunk.data instanceof Uint8Array) {
            this.chunkTmp.push(chunk);
            if (chunk.meta.percent !== 100) {
                return;
            }
            var content = this.encryptContent(this.chunkTmp, this.previous.streamInfo.compressedSize);
 ...
 */
ZipFileWorker.prototype.encryptContent = function (contents, size) {
    //the file which size small than 16384B no need new Uint8Array.
    if (contents.length === 1) {
        return aesWorker.encryptWorker(this.encryptOptions).processData(contents[0]);
    }
    var content = new Uint8Array(size);
    var offset = 0;
    for (var i = 0; i < contents.length; i++){
        var buffer = contents[i];
        content.set(buffer.data, offset);
        offset += buffer.data.length;
    }
    return aesWorker.encryptWorker(this.encryptOptions).processData(content);
};
...
ZipFileWorker.prototype.closedSource = function (streamInfo) {
    ...
    if (this.encryptOptions.password !== null && !streamInfo['file'].dir) {
        if (!this.streamFiles) {
            var content = this.encryptContent(this.contentBuffer, streamInfo.compressedSize);
