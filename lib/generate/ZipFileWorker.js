...
ZipFileWorker.prototype.closedSource = function (streamInfo) {
    ...
    if (this.encryptOptions.password !== null && !streamedContent) {
        var content = new Uint8Array(streamInfo.compressedSize);
        var offset = 0;
        while (this.contentBuffer.length) {
            var buffer = this.contentBuffer.shift();
            content.set(buffer.data, offset);
            offset += buffer.data.length;
        }
        content = aesWorker.encryptWorker(this.encryptOptions).processData(content);
